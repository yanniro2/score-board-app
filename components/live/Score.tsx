...
const Page: React.FC<ScoreDashboardProps> = ({ id }) => {
  ...
  return (
    <div className="container h-[30%] justify-evenly gap-[2rem] mx-auto  text-white shadow-xl  rounded-xl text-center flex flex-col   xl:text-[1rem] 2xl:text-[6rem] ">
      {responseData ? (
        <div className="flex w-[80%] items-end justify-around h-full mx-auto">
          <Team
            teamName={responseData.success.match.team_one_name}
            teamLogo={responseData.success.match.team_one_image_url}
          />

          <div className="flex items-center flex-col justify-end  mx-auto 2xl:text-[8rem] h-full w-1/3 gap-[2rem]">
            <ScoreTotal id={id} />
